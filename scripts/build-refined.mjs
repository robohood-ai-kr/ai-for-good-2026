...
const galleryStyle=`body{font:17px/1.6 system-ui,sans-serif;color:#0f172a;background:#f4f7fb;margin:0;padding:32px}main{max-width:1400px;margin:auto}a{color:#1d4ed8}h1{font-size:32px}p{color:#475569}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:24px}.card{background:white;border:1px solid #d5dde7;border-radius:12px;padding:18px;min-width:0}.card img{width:100%;height:230px;object-fit:contain;background:#f4f7fb;border:1px solid #e2e8f0;border-radius:6px}.card h2{font-size:18px;margin:16px 0 8px}.links{display:flex;gap:16px;flex-wrap:wrap}.meta{font-size:14px;color:#64748b}.hero{padding:24px;background:white;border:1px solid #d5dde7;border-radius:12px;margin-bottom:24px}svg{width:25px;height:25px;vertical-align:middle}.button{display:inline-block;background:#2563eb;color:white;text-decoration:none;padding:12px 18px;border-radius:8px;margin:16px 10px 0 0}@media(max-width:600px){body{padding:20px 16px}.grid{grid-template-columns:1fr}h1{font-size:27px}}`;
function html(title,content) {return `<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>${galleryStyle}</style><main>${content}</main></html>`;}
function gallery(sector,online=true) {
 const c=sectors[sector];
 const start=online?'./index.html':`http://127.0.0.1:4173/${sector}/`;
 return html(`${c.name} v1 · 새 생성 이미지`,`<div class="hero"><h1>${c.name} · v1 개선 시안</h1><p>이전 이미지의 간결한 작업 흐름 + Stitch의 파란색 운영 UI. 새 생성 시안을 기준으로 실제 HTML/CSS를 구현했습니다. 이미지의 예시 수치·문구가 기능 보장을 뜻하지 않습니다.</p><div class="links"><a href="${start}">실행 앱</a><a href="${online?`./${c.prefix}-03.html`:'./prompts.json'}">${online?'대시보드':'프롬프트 원문'}</a></div></div><div class="grid">${c.names.map((name,i)=>{const id=screenId(sector,i+1),img=online?`./references/${id}.png`:`./${sector}/${id}.png`;return `<article class="card"><a href="${img}" target="_blank" rel="noopener"><img loading="lazy" src="${img}" alt="${e(specFor(sector,i+1).title)} 생성 시안"></a><p class="meta">${id.toUpperCase()} · ${c.mobile.includes(i+1)?'모바일':'데스크톱'}</p><h2>${e(name)}</h2><div class="links"><a href="${img}" target="_blank" rel="noopener">원본 PNG</a><a href="${online?`./${id}.html`:`http://127.0.0.1:4173/${sector}/${id}.html`}">구현 화면 ↗</a></div></article>`}).join('')}</div>`);
}
for(const sector of selected) {
 ...
 for(let n=1;n<=c.names.length;n++) {
  ...
  await writeFile(path.join(dest,`${id}.html`),renderPage(sector,n));
...
await writeFile(path.join(root,'dist/index.html'),html('RoboHood · 두 가지 v1',`<div class="hero"><p>ROBOHOOD · IMAGE → CODE</p><h1>${icon('robot')} 두 가지 v1, 더 선명한 현장 경험</h1><p>기능은 제품별로 분리하고, 쉬운 설명과 명확한 작업 흐름·파란색 운영 UI를 결합했습니다.</p><p class="meta">제조업 21개 · 소상공인 20개 화면. 브라우저 기록만 저장하는 프런트엔드 데모입니다.</p></div><div class="grid">${Object.entries(sectors).map(([sector,c])=>`<article class="card"><a href="./${sector}/${c.prefix}-03.html"><img src="./${sector}/references/${c.prefix}-03.png" alt="${c.name} 새 대시보드 생성 시안"></a><h2>${c.name} v1</h2><p>${c.description}</p><a class="button" href="./${sector}/${c.prefix}-03.html">대시보드 열기 →</a><a href="./${sector}/image-gallery.html">${c.names.length}개 새 이미지</a><p class="links"><a href="./${sector}/">서비스 소개</a><a href="./${sector}/${c.prefix}-${sector==='manufacturing'?'17':'11'}.html">현장 모바일</a></p></article>`).join('')}</div>`));
...
await writeFile(path.join(concepts,'index.html'),html('RoboHood v1 개선 이미지',`<h1>RoboHood · v1 개선 이미지</h1><p>이전 imagegen + Stitch 장점을 합친 두 제품의 독립 시안입니다.</p><div class="grid">${Object.entries(sectors).map(([s,c])=>`<article class="card"><img src="./${s}/${c.prefix}-03.png" alt="${c.name} 대시보드"><h2>${c.name} · ${c.names.length}화면</h2><a href="./${s}.html">전체 생성 이미지 보기</a></article>`).join('')}</div><p><a href="./prompts.json">화면별 생성 프롬프트</a></p>`));
