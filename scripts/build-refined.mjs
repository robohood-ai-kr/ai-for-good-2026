...
function html(title,content,fontHref,faviconHref='') {return `<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title>${faviconHref?`<link rel="icon" href="${faviconHref}" type="image/svg+xml">`:''}${fontHref?`<link rel="stylesheet" href="${fontHref}">`:''}<style>${galleryStyle}</style><main>${content}</main></html>`;}
...
for(const sector of selected) {
 ...
 await copyFile(path.join(root,'assets/favicon.svg'),path.join(dest,'assets/favicon.svg'));
