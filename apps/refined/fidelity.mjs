...
export function fidelityContent(sector, number, h) {
  ...
  switch (s.kind) {
    ...
    case 'explorer': {
      ...
      const filters = card('',`<div class="rh-explorer-filters">${filter(mf?'지역':'생활권',['전체',mf?'제조 권역':'서울 생활권'])}${filter(mf?'공정·업종':'업종',['전체',mf?'금속가공':'음식점',mf?'조립':'동네마트','확장 업종'])}${filter('데이터 유형',['전체',mf?'RGB 이미지':'1인칭 영상','텔레오퍼레이션 구간'])}${filter('수집 상태',['전체','수집 가능','준비 중'])}</div><div class="rh-explorer-search"><label class="rh-search">${icon('search')}<input type="search" data-search aria-label="현장 검색" placeholder="현장명·업무·데이터 검색"></label><label class="rh-toggle"><input type="checkbox" data-hide-map>지도 숨기기</label></div>`,'rh-filter-card');
      if (!mf) {
        const scenarioCards = Object.entries(c.scenarios).map(([id,item],index)=>`<article class="rh-card rh-site-item ${index===0?'rh-selected-site':''}" data-search-item data-scenario-card="${id}" data-site-category="${e(item.siteCategory)}" data-site-state="수집 가능"><figure class="rh-photo compact"><img src="./assets/scenarios/${e(item.image)}" alt="${e(item.alt)}"><figcaption>현장 업무 이미지</figcaption></figure><div class="rh-site-description"><div class="rh-title-line"><h3>${e(item.site)}</h3>${badge('수집 가능','blue')}</div><p>${e(item.title)}</p><div class="rh-tags">${badge(e(item.dataType))}${badge('범위 협의 필요')}</div><section class="rh-site-conditions"><h3>수집 시 지켜야 할 조건</h3><ol><li>${e(item.conditions)}</li><li>${e(item.safety)}</li><li>현장 담당자와 촬영 시간·범위를 사전에 확정합니다.</li></ol></section>${actions(`<a class="rh-button" href="${route(sector,5)}" data-scenario-link="${id}">현장 상세</a><a class="rh-button rh-primary" href="${route(sector,c.newPage)}" data-scenario-link="${id}">수집 요청</a>`)}</div></article>`).join('');
      ...
      const list = `<section class="rh-site-list"><div class="rh-site-list-heading"><h2>현장 목록 <span>3</span></h2><small>가상 예시 · 실제 모집 아님</small></div>${sites.map((name,i)=>`<article class="rh-card rh-site-item ${i===0?'rh-selected-site':''}" data-search-item data-site-category="${i===0?(mf?'금속가공':'카페'):i===1?(mf?'금속가공':'베이커리'):mf?'조립':'기타'}" data-site-state="${i===0?'협의 필요':'협의 전'}">${i===0?photo(mf,'compact'):!mf?scenePhoto(sector,number,'compact',i):`<div class="rh-site-thumb">${icon(i===2?'assignment':'explore')}<small>모의 현장</small></div>`}<div class="rh-site-description"><div class="rh-title-line"><h3>${name}</h3>${badge(i===0?'선택':'협의 전',i===0?'blue':'')}</div><p>${i===0?(mf?'소형 부품 외관 검사 · 비가동 작업대':'트레이 위 컵 정리 · 모의 실습 공간'):'수집 조건·허용 업무 미정'}</p><div class="rh-tags">${badge(i===0?(mf?'RGB 이미지':'1인칭 영상'):'조건 미정')}${badge(i===0?'범위 협의 필요':'미모집')}</div>${!mf&&i===0?'<section class="rh-site-conditions"><h3>수집 시 지켜야 할 조건</h3><ol><li>동의한 범위에서 손과 물체만 촬영</li><li>빈 비파손 컵과 트레이만 사용</li><li>고객·얼굴·개인정보 촬영 금지</li></ol></section>':''}${i===0?actions(link(sector,mf?6:5,'현장 상세')+link(sector,c.newPage,'수집 요청',true)):'<p class="rh-hint">탐색 화면의 배치 예시입니다.</p>'}</div></article>`).join('')}${notice('실제 사업장 위치·모집 현황·협약을 나타내지 않습니다. 선택된 한 개 과제만 서비스 흐름에서 연결됩니다.')}</section>`;
      return mf?`${filters}<div class="rh-map-layout">${regionMap()}${list}</div>`:`<div class="rh-map-layout"><div class="rh-local-map-panel">${filters}${sceneMap(false)}</div>${list}</div>`;
    }
